  ...
  try {
    ...
    if (holdingsList.length === 0) {
      ...
      return {
        status: 'error',
        message: isDegraded
          ? 'Unable to retrieve portfolio data — the portfolio service encountered an error. Cannot compute scenario impact, but the user may have holdings that could not be loaded.'
          : 'Portfolio is empty — cannot compute scenario impact.',
