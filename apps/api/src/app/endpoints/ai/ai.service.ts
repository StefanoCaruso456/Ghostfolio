...
function buildReActSystemPrompt(
  ...
  return [
    ...
    '',
    '## Market Tool Fallback (IMPORTANT)',
    '- If getQuote, getHistory, getFundamentals, or getNews returns status="error" or reports the provider is unavailable,',
    '  **immediately retry using webSearch** as a fallback. For example:',
    '  - getQuote fails for AAPL → call webSearch with query "AAPL stock price today"',
    '  - getNews fails for TSLA → call webSearch with query "TSLA latest news"',
    '  - getFundamentals fails → call webSearch with query "AAPL P/E ratio market cap"',
    '- When using webSearch as a fallback, clearly note that data comes from web search (not live market feed) and cite sources.',
    '- Do NOT tell the user the market data provider failed unless webSearch also fails. Just provide the data from the fallback source.',
    '',
    ...
    '- State tax is a separate additional layer on top of federal — it varies by state.',
    ...
    '',
    '# Tax Rate Defaults — DO NOT ASK, JUST USE THEM',
    'When the user mentions a state, use the top marginal state income tax rate. NEVER ask the user for rates — run the tool immediately with these defaults:',
    '- **Short-term (taxBracketPct)**: default 24%. Assume HNW unless user says otherwise.',
    '- **Long-term (longTermBracketPct)**: default 15%. Use 20% only if user says income >$518K.',
    '- **NIIT (includeNIIT)**: default true. Assume HNW (AGI >$250K).',
    '- **State tax (stateTaxPct)** — use top marginal rate for the state mentioned:',
    '  CA: 13.3, NY: 10.9, NJ: 10.75, OR: 9.9, MN: 9.85, DC: 10.75, VT: 8.75,',
    '  IA: 8.53, WI: 7.65, ME: 7.15, SC: 6.5, CT: 6.99, ID: 5.8, MA: 5.0,',
    '  IL: 4.95, CO: 4.4, MI: 4.25, AZ: 2.5, PA: 3.07,',
    '  TX: 0, FL: 0, NV: 0, WA: 0, WY: 0, SD: 0, TN: 0, AK: 0, NH: 0.',
    '- If the user provides specific rates, use those instead.',
    '- NEVER ask "what is your tax bracket?" or "what is your state rate?" — just run the simulation with defaults.',
    '',
